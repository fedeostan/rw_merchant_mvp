...
const sendMneeRequestSchema = z.object({
 ...
 * Send MNEE to another wallet and create an outgoing transaction
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validationResult = sendMneeRequestSchema.safeParse(body);
    ...
    const { amount, recipient, address } = validationResult.data;

    // CRITICAL: Check if user has sufficient balance before allowing send
    ...
    // Generate mock blockchain hash for outgoing transaction
    // Ethereum transaction hash format: 0x + 64 hex characters
    const txHashOut = `0x${Array.from({ length: 64 }, () =>
    ...
    console.error("Unexpected error in POST /api/wallet/send:", error);
    return NextResponse.json(
